...
function errorDirective()
{
    return {
        ...
        replace: true,
        transclude: false,
        template: '<section class="comment-error error">! <span ng-show="{{errorCode}}">Failed to {{operation}}: [{{errorCode}}] {{errorMessage}}.</span><span ng-show="{{!errorCode}}">{{error}}</span></section>',
        scope: {
            operation: '=operation',
            errorCode: '=code',
